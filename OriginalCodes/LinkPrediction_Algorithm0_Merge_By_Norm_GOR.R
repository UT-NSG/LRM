...
LinkPrediction_Algorithm0_Merge_By_Norm_GOR <- 
  ...
  if(includeOnlyLRM == 0) {
    ...
    ###########################
    # Normalizing 
    ScoresMatrix = (ScoresMatrix-min(ScoresMatrix))/(max(ScoresMatrix)-min(ScoresMatrix))
  ...
  if(is.na(auxiliaryLayers) == FALSE) {
    for(iAuxL in 1:length(auxiliaryLayers)){
      ...
      ###########################
      # Normalizing 
      AUX_score = (AUX_score-min(AUX_score))/(max(AUX_score)-min(AUX_score))
      
      ScoresMatrix = ScoresMatrix + GOR(A , AUX_Matrix) * AUX_score
    }
  }
