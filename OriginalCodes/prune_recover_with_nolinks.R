...
source("prune_predict_assess_with_nolinks.R")
prune_recover_with_nolinks <- function (g, ..., probes = seq(0.1, 0.9, 0.1), epochs = 10, 
          preserve_conn = FALSE, use_weights = FALSE) 
{
  ...
  evaluation <- map2(as.list(res$method), as.list(res$links_rem),
                     prune_predict_assess_with_nolinks, g = g, removable_links = removable_links)
