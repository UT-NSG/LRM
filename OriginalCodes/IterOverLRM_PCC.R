    ...
    Assessed =  tryCatch({
      Assessed =
        ComparingMethodsCallerFunctionWrapper(
          Target_Layer_Graph, 
          method = "LinkPrediction_Algorithm0_Merge_By_PCC", 
...
write.table(Results_Matrix, 'SPM(Self)LRM(AUX)PCC_mean_test.txt', row.names = TRUE)
# proper naming scheme
# atl1_atl2_13981008_mean.txt
write.table(Results_Matrix_SD, 'SPM(Self)LRM(AUX)PCC_std_test.txt', row.names = TRUE)
# proper naming scheme
