...
source('ComparingMethodsCallerFunction_Masked_LRM_Compatible.R')
...
AggregatedAssesment = 
  ComparingMethodsCallerFunction_Masked_LRM_Compatible(
    Target_Layer_Graph, 
    method = "Masked_LRM_Algorithm", 
    ReconstructionList_Graph = NA, #ReconstructionList_Graph, 
    LegendList,
    includeSelf = 1, # if 1, includes the self, if 0, won't include the self. Put 1 and NA as auxillary layers to only use self 
    probesOfLinks = seq(0.1, 0.9, 0.1), 
    NoLinksToLinksRatio = 2, # this arguement determines the ratio between no links and links
...
saveRDS(AggregatedAssesment, file = 
          "RDS_Data/AggregatedAssesment_Air-Train-Air-k=30-MLRM.rds"
        )
